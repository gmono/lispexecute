import { Table, LispObject, LispSymbolRefence, Store, LispDefProcess, LispRawProcess } from "./DataType";
import { Executer, SymDecorator } from "./Execute";
import { DWord, LimitNumber } from "./LimitNumber";
...
export class LispExecuter extends Executer {
    ...
    public ROL(...args) {
        ...
        let tbits = new DWord(true, num).GetLimitBits(32 - num2, 31);
        let ret = num << num2;
        let temp = new DWord(false, ret);
    ...
    public ROR(...args) {
        ...
        //使用LimitNumber库处理
        if (num2 == 0) return num;
        let tbits = new DWord(true, num).GetLimitBits(0, num2 - 1);
        let ret = num >> num2;
        let temp = new DWord(false, ret);
